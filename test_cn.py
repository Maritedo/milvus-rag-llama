...
def evaluate(results_file, results, model, embedder, title, identifier=None, numofexps=0, fixed=False, quite=False, batch_size=1):
    ...
    for key in results.keys():
        ...
        total[2] += count_contained_intervals(target["entities"], generated["entities"])[0]
            ...
            while True:
                ...
                for next in range(next, end):
                    ...
                    _last_successed, _interrupted, output = evaluation_worker(
                        ...
                        quite=quite)
                    if _last_successed:
                        matched, recall, precision = count_contained_intervals(test_sentences[next]["entities"], output["entities"])
                        total[0] += len(test_sentences[next]["entities"])
...
def main():
    ...
    for index, task in enumerate(tasks):
        ...
        identifier = f"{embedder}_{model.replace(':', '-')}_{nexps}exps{'_fixed' if fixed else ''}"
