...
def evaluate(results_file, results, model, embedder, numofexps, fixed, quite, title):
    ...
    for index in range(len(results)):
        ...
        total[2] += re2[0]
    with DynamicDisplay(4, extra=[logs_scroll], extra_height=16) as display:
        def log(msgs, flag=-1): # -1 means logging, otherwise print to logs list but do not print to console and setline
            if flag == -1:
                for msg in msgs.split("\n"):
                    ...
                    logs.append((msg, flag, ))
            else:
                display.set_line(flag, msgs)
                for msg in msgs:
                    logs.append((msg, flag, ))
        display.set_line(0, title)
        status_line = DynamicText(lambda: f"[Entities] Expected:{total[0]}  Generated:{total[1]} Matched:{total[2]}")
        display.set_line(1, status_line)
        statistics_line = DynamicText(lambda: f"Recall: {(total[2] / (total[0]) if total[0] else 0) * 100:.2f}%, Precision: {(total[2] / total[1] if total[0] else 0) * 100:.2f}%")
